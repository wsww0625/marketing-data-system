  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="page-header mb-0">批次管理</h2>
      ...
      <div className="card">
        <DataTable
          columns={[
            ...
            { key: 'filtersJson', title: '筛选条件', render: (r: any) => <span className="text-xs text-gray-500">{parseFilters(r.filtersJson)}</span> },
            { key: 'total', title: '号码数量' },
            { key: 'isCoolExport', title: '类型', render: (r: any) => (
              <span className={r.isCoolExport ? 'badge-info' : 'badge-default'}>{r.isCoolExport ? '冷却导出' : '全量导出'}</span>
            )},
            { key: 'status', title: '状态', render: (r: any) => (
              <span className={r.status === 'completed' ? 'badge-success' : r.status === 'failed' ? 'badge-error' : 'badge-warning'}>
                {r.status === 'completed' ? '完成' : r.status === 'failed' ? '失败' : `处理中 ${r.progress}%`}
              </span>
            )},
            { key: 'coolReverted', title: '冷却状态', render: (r: any) => (
              r.isCoolExport ? (
                <span className={r.coolReverted ? 'badge-default' : 'badge-warning'}>{r.coolReverted ? '已解除' : '冷却中'}</span>
            ...
            { key: 'actions', title: '操作', render: (r: any) => (
              <div className="flex gap-3">
                {r.status === 'completed' && r.filePath && (
                  <a href={getDownloadUrl(r.filePath)} className="text-blue-600 text-sm hover:text-blue-700 font-medium">下载</a>
                )}
                {r.isCoolExport && !r.coolReverted && r.status === 'completed' && (
                  <button onClick={() => { if (confirm('确认解除冷却？')) revertCoolingMutation.mutate({ batchId: r.id }); }}
                    className="text-amber-600 text-sm hover:text-amber-700 font-medium">解除冷却</button>
                )}
              </div>
            )},
          ]}
